...
import { prependOrder } from "../api/orders.js";
...
export function CheckoutPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_res, orderPayload) => {
      const newOrder = {
        id: makeOrderId(),
        eventName: `Same-day order — ${orderPayload.fullName}`,
        total: orderPayload.total,
        status: "PREPARING",
        date: todayISO(),
        deliveryWindow: orderPayload.deliveryWindow,
        eco: orderPayload.items?.every((i) => i.ecoOnly) ?? false,
        ...
        updates: [
          ...
          ...(orderPayload.note?.trim()
            ? [
                {
                  time: nowHHMM(),
                  text: `Customer note: ${orderPayload.note.trim()}`,
                },
              ]
            : []),
        ],
      ...
      const nextOrders = prependOrder(newOrder);
